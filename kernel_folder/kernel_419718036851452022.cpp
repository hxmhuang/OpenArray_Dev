...
extern "C" {
void kernel_419718036851452022(void** &list, int o) {
  //o = 1;//temp wangdong
  oa_int3* oa_int3_p = (oa_int3*)(list[3]);


  const float F_0 = ((float*)list[0])[0];
        ...
        for (int k = kst; k < ked; k++) {
          for (int j = jst; j < jed; j++) {
            ...
            for (int i = ist; i < ied ;i++){
              list_2[calc_id2(i,j,k,S1_0,S1_1)] = (F_0)*(1.0*((list_1[calc_id2(1+i,j,k,S0_0,S0_1)])-(list_1[calc_id2(i,j,k,S0_0,S0_1)])));
            }
          }
        }
      }
